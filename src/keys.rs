...
use crate::error::{INVALID_SECRET_KEY_SER, NULL_POINTER, OK, SIGNING_ERROR, STR_CONV_ERR};
...
mod test {
    ...
    pub fn test_random_keypair_with_valid_params() {
        ...
        assert_ne!(priv_key, priv_key_before);
        let key = RistrettoSecretKey::from_bytes(Scalar::from_bytes_mod_order(priv_key).as_bytes()).unwrap();
        assert_eq!(RistrettoPublicKey::from_secret_key(&key).as_bytes(), pub_key);
    }
